...
- 推进器：`/home/mifu/openclaw-data/skills/script-first-testing/scripts/promoter.sh`
- 看门狗：`/home/mifu/openclaw-data/skills/script-first-testing/scripts/watchdog.sh`
- cron 安装脚本：`/home/mifu/openclaw-data/skills/script-first-testing/scripts/install_watchdog_cron.sh`
...
```bash
bash /home/mifu/openclaw-data/skills/script-first-testing/scripts/promoter.sh
```
...
```bash
bash /home/mifu/openclaw-data/skills/script-first-testing/scripts/install_watchdog_cron.sh
```

机制：
- 推进器负责持续推进和刷新状态
- 看门狗负责检查推进器最近 15 分钟是否有心跳
- 若无心跳，则自动唤醒推进器
- cron 默认每 10 分钟轮询一次
